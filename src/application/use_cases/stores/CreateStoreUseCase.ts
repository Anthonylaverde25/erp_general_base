...
@injectable()
export class CreateStoreUseCase implements IUseCase<CreateStoreDTO, { store: StoreEntity; message: string }> {
	constructor(
		@inject(TYPES.IStoreRepository)
		private readonly repository: IStoreRepository
	) {}

	async execute(data: CreateStoreDTO): Promise<{ store: StoreEntity; message: string }> {
