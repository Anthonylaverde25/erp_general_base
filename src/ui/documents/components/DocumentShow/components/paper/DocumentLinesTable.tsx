...
const getSourceDocumentLabel = (line: DocumentLine, predecessors?: ParentDocumentInfo[]) => {
				...
				<TableBody>
					{pageLines.map((line, rowIndex) => {
						...
						const isFirstOfGroup =
							!prevLine || prevLine.source_document_number !== line.source_document_number;
						const progress = line.quantity ? (line.processed_quantity / line.quantity) * 100 : 0;
						const isDone = progress >= 99.9;

						const showGroupHeader =
							isFirstOfGroup && (line.source_document_number || line.source_document_id);
						...
						return (
							<Fragment key={line.id || `${line.name}-${rowIndex}`}>
								...
								<TableRow
									sx={{
										'&:nth-of-type(odd)': {
											backgroundColor: isDark ? 'rgba(255,255,255,0.01)' : '#fcfdfe'
										},
										'&:hover': {
											backgroundColor: isDark
									...
									>
										{!isProcessableDocType ? (
											<Typography style={{ fontSize: '13px', fontWeight: 700 }}>
												{isCreditNote ? -line.quantity : line.quantity}
											</Typography>
										) : (
											<Tooltip
												title={`Procesado: ${isCreditNote ? -line.processed_quantity : line.processed_quantity} de ${isCreditNote ? -line.quantity : line.quantity}`}
											>
												<Box className="flex w-full flex-col px-2 py-1">
													<Box className="mb-0.5 flex items-baseline justify-between">
