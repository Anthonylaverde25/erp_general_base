		...
		>
			...
			<Box className="mb-2 flex flex-wrap gap-2">
				...
				<Button
					...
					onClick={() => {
						if (defaultContact?.email) window.location.href = `mailto:${defaultContact.email}`;
					}}
					sx={actionBtnSx}
				>
					Email
				...
				<Button
					...
					disabled={!defaultContact?.phone}
					onClick={() => {
						if (defaultContact?.phone) window.location.href = `tel:${defaultContact.phone}`;
					}}
					sx={actionBtnSx}
				>
					Llamar
			...
			{defaultAddress ? (
				<Box className="flex items-start gap-3">
					<Avatar
						sx={{ width: 32, height: 32, bgcolor: 'action.selected', color: 'text.secondary', mt: 0.5 }}
					>
					...
					<Box sx={{ minWidth: 0, flex: 1 }}>
						...
						>
							{defaultAddress.country && (
								<span className="font-semibold">{defaultAddress.country}, </span>
							)}
...
function FichaTecnicaDialog({
	...
	const allData = {
		...
		banks:
			partner.bank_accounts?.map((acc, i) => ({
