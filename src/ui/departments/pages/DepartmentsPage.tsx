...
export default function DepartmentsPage() {
    ...
    const { selectedDepartment, breadcrumbPath } = useMemo(() => {
        if (!departments || departments.length === 0) return { selectedDepartment: null as IDepartment | null, breadcrumbPath: [] as { title: string, url: string, code: string }[] };
        ...
        return {
            ...
            breadcrumbPath: [{ title: defaultDept.name, url: `/departments/${defaultDept.code}`, code: defaultDept.code }]
        };
    }, [departments, code]);
    ...
    const activeRootTabCode = breadcrumbPath.length > 0 ? breadcrumbPath[0].code :
        (departments && departments.length > 0 ? departments[0].code : '');
    ...
    return (
        <Root
            ...
            content={
                <Box className="w-full h-full p-4 overflow-y-auto">
