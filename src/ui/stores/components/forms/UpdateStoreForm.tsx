...
import { TextField, Button, Box, Typography, Divider, Stack, Fade, Dialog } from '@mui/material';
...
import { UpdateStoreFormType, updateStoreSchema } from '@/schemas/store/store.schema';
...
export default function UpdateStoreForm({ storeId, onCancel, onSuccess }: UpdateStoreFormProps) {
	const { handleUpdateStore, isLoading } = useUpdateStore();
	const { store, isLoading: isLoadingStore } = useShowStore(storeId);
									...
									>
										<Box p={3}>
											...
											<Typography
												...
												sx={{ mb: 3 }}
											>
												¿Estás seguro de que deseas eliminar la dirección de esta tienda? Esta
