...
} from "@/schemas/categories/categories.defaults";
import {
    Button,
    TextField,
    Box,
...
export function CategoriesForm({ mode, data, initialParent, onCancel, onSuccess }: CategoriesFormProps) {
    ...
    const availableParents = allCategories?.filter(cat =>
        !data || (cat.id !== data.id && cat.parent_id !== data.id)
    ) || [];

    const { data: categoryData, isLoading: isLoadingCategory } = useShowCategory(
        data?.id || null
    );
    ...
    const isLoading =
        createCategory.isPending || updateCategory.isPending || isLoadingCategory;
