...
export default function AddressForm({ prefix }: AddressFormProps) {
	...
	const renderTextField = (name: string, label: string, placeholder: string, autoFocus = false) => (
		<Controller
			name={getName(name)}
			control={control}
	...
	return (
		<Box sx={{ mt: 2 }}>
			{/* Calle full width */}
			<Box mb={2}>{renderTextField('street', 'Calle y número', 'Ej: Av. Libertador 1234', true)}</Box>
