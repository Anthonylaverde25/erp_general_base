...
import { familySchema, FamilyFormType } from '@/schemas/families/families.schema';
import { defaultCreateFamilyValues, defaultUpdateFamilyValues } from '@/schemas/families/families.defaults';
import { Button, TextField, Box, Typography, Divider, Stack, Fade, Autocomplete, Chip } from '@mui/material';
...
export function FamiliesForm({ data, onCancel, onSuccess }: FamiliesFormProps) {
	...
	const { data: familyData, isLoading: isLoadingFamily } = useShowFamily(data?.id || null);
	...
	const isLoading = createFamily.isPending || updateFamily.isPending || isLoadingFamily;
		...
		<Fade in={true}>
			...
			>
				...
				<Box>
					...
					>
						{data ? 'Update the family information below.' : 'Enter the details for the new family.'}
					</Typography>
				</Box>
					...
					<Controller
						...
						render={({ field }) => (
							<Autocomplete
								...
								value={taxRates?.filter((rate) => field.value?.includes(rate.id)) || []}
								onChange={(_, newValue) => {
