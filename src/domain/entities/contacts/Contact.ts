...
export class ContactEntity implements IContact {
	...
	static fromPrimitives(data: IContact): ContactEntity {
		return new ContactEntity(data.id, data.email, data.phone, data.default || false);
	}

	static create(data: CreateContactDTO): ContactEntity {
		return new ContactEntity(undefined, data.email, data.phone, data.default || false);
	}

	static update(id: number, data: Partial<ContactEntity>): ContactEntity {
		return new ContactEntity(id, data.email!, data.phone, data.default_contact!);
	}
