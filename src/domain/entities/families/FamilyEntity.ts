...
export class FamilyEntity implements Family {
	...
	static fromPrimitives(data: Family): FamilyEntity {
		return new FamilyEntity(
			...
			data.is_active,
			data.tax_rates ? data.tax_rates.map((rate) => TaxRateEntity.fromPrimitives(rate)) : undefined
		);
	}

	static create(data: CreateFamilyDTO): FamilyEntity {
		return new FamilyEntity(0, 0, data.tax_rate_ids, data.name, data.percentage, data.is_active, undefined);
	}
	...
	toPlainObject(): Family {
		return {
			...
			tax_rates: this._tax_rates ? this._tax_rates.map((rate) => rate.toPlainObject()) : undefined
		};
	}
}
