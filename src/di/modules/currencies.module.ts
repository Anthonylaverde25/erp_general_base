...
export function registerCurrenciesModule(container: Container) {
	// Repository
	container.bind<ICurrencyRepository>(TYPES.ICurrencyRepository).to(CurrencyRepositoryCrud);

	// Use Cases
	container.bind<IndexCurrenciesUseCase>(TYPES.IndexCurrenciesUseCase).to(IndexCurrenciesUseCase);
}
