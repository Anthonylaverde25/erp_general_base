...
export default function useCreateTaxType() {
	...
	const use_case = container.get<CreateTaxTypeUseCase>(TYPES.CreateTaxTypeUseCase);

	const { mutateAsync: handleCreateTaxType, isPending: isLoading } = useMutation({
		...
		onError: (error: any) => {
			const message = error?.response?.data?.message || 'Error al crear el tipo de impuesto';
			toast.error(message);
		}
	});
