...
export class TaxTypeRepositoryCrud implements ITaxTypeRepository {
    async index(): Promise<TaxTypeEntity[]> {
        const { data: { tax_types } } = await axiosInstance.get(`tax-types`);
    ...
    async update(id: TaxType['id'], data: Partial<TaxTypeEntity>): Promise<{ tax_type: TaxTypeEntity; message: string }> {
        const {
            data: { tax_type, message }
