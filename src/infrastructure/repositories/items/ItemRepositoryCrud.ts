...
export class ItemRepositoryCrud implements IItemRepository {
	...
	private appendFormData(formData: FormData, key: string, value: unknown): void {
		if (value === undefined) {
			return;
		}
	...
	async create(itemData: ItemEntity): Promise<{ item: ItemEntity; message: string }> {
	...
	async update(id: number, itemData: ItemEntity): Promise<{ item: ItemEntity; message: string }> {
