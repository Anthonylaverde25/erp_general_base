...
export class CompanyRepositoryAction implements ICompanyActionRepository {
	async changeCompany(id: ICompany['id']): Promise<string> {
		const {
			data: { message }
		} = await axiosInstance.post('companies/change-active-company', { companyId: id });
		return message;
	}
	...
	async createAddress(companyId: number, data: AddressEntity): Promise<{ address: AddressEntity; message: string }> {
		...
		} = await axiosInstance.post(`companies/${companyId}/addresses`, data.toPlainObject());
	...
	async createBankAccount(data: BankAccountEntity): Promise<{ bank_account: BankAccountEntity; message: string }> {
		...
		} = await axiosInstance.post(`companies/bank-accounts`, data.toPlainObject());
	...
	async changeDefaultAddress(addressId: number): Promise<{ status: number; message: string }> {
		const {
			data: { message },
			status
		} = await axiosInstance.patch('companies/change-default-address', { address_id: addressId });
	...
	async changeDefaultContact(contactId: number): Promise<{ status: number; message: string }> {
		const {
			data: { message },
			status
		} = await axiosInstance.patch('companies/change-default-contact', { contact_id: contactId });
		return { status, message };
	}
}
